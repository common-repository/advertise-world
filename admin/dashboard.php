<?php
/**
 * Advertise World admin dashboard page
 *
 * Displays and manages functionality for the admin dashbaord page.
 *
 * @link https://www.advertiseworld.com
 *
 * @package WordPress
 * @subpackage Advertise_World_Admin_Dashboard
 * @since 1.0.0
 */


/**
 * Displays Advertise world admin dashboard page
 *
 * @since 1.0.0
 * @since 1.0.5 Uses the Advertise_World_Send_Email_To_Support function to send emails which is more reliable then wp_mail function.
 * @since 1.1.0 Images are inline base64, Ad Blocker plus was causing issues with images loading.
 */
function advertise_world_wp_admin_menu() {

	$options     = get_option( 'advertise-world-wp-options-account' );
	$sent_errors = 0;

	if ( isset( $_POST['advertise_world_support']['email'] ) ) {

		$email   = trim( $_POST['advertise_world_support']['email'] );
		$name    = trim( $_POST['advertise_world_support']['name'] );
		$message = trim( $_POST['advertise_world_support']['message'] );
		if ( '' === $message ) {
			$sent_errors ++;
			add_settings_error(
				'mail-msg-invalid',
				'settings_updated',
				'Please enter a message to send an email!',
				'error' );
		}
		if ( is_email( $email ) ) {
			require_once( dirname( __FILE__ ) . "/../advertise-world-wp.php" );
			$content = "\r\n" . $message;
			$content .= "\r\n\r\n From: " . $email;
			$content .= "\r\n Name: " . $name;
			$content .= "\r\n URL: " . home_url();
			$mail_sent = Advertise_World_Send_Email_To_Support( 'Support for ' . home_url(), $content, $email );
			if ( ! $mail_sent ) {
				$sent_errors ++;
				add_settings_error(
					'mail-send-fail',
					'settings_updated',
					'An error has occured and the email has not been sent. Please contact us manually <a href="mailto:support@advertiseworld.com">here</a> ',
					'error' );
			}
		} else {
			$sent_errors ++;
			add_settings_error(
				'mail-email-invalid',
				'settings_updated',
				'Please enter a valid email!',
				'error' );
		}
	}

	if ( isset( $_POST['advertise_world_support']['email'] ) && $sent_errors === 0 ) {
		add_settings_error(
			'account-valid',
			'settings_updated',
			'Thank you for your message we will respond shortly!',
			'updated' );
	}

	$existing_account_email = '';
	if ( is_array( $options ) && array_key_exists( 'account', $options ) ) {
		$existing_account_email = $options['account'];
	}

	?>
	<div class="wrap" style=" margin-left: 5%;">
	<img class="advertise-world-contact-us-logo"
	     src="data:image/png;base64,">
	<h2 class="nav-tab-wrapper wp-clearfix" style="margin-bottom: 35px;"><a href="/wp-admin/nav-menus.php"
	                                                                        class="nav-tab nav-tab-active">Advertise
			World Account Settings</a></h2>

	<div class="advertise-world-admin-container">
	<?php settings_errors(); ?>
	<form method="post" action="options.php">
		<?php settings_fields( 'advertise-world-wp-options-account' ); ?>
		<p>Please Link your Advertise World Account here:</p>
		<table scope="form-table" class="advertise-world-contact-us-table">
			<td class="row advertise-world-contact-us-label">
				<p>Publisher Account Email</p>
			</td>
			<td class="advertise-world-contact-us-field">
				<input type="text" name="advertise-world-wp-options-account[account]"
				       id="advertise-world-wp-options-main-account" value="<?php echo $existing_account_email; ?>"/>
			</td>
		</table>
		<input name="Submit" type="submit" value="<?php esc_attr_e( 'Save Changes' ); ?>"
		       class="button button-primary"/>
	</form>

	<br/>
	<br/>

	<p>Contact Us</p>
	<table scope="form-table">

		<form action="" method="post">
			<?php settings_fields( 'advertise-world-wp-options-account' ); ?>
			<table class="advertise-world-contact-us-table">
				<tr>
					<td class="advertise-world-contact-us-label"><label>your email</label></td>
					<td class="advertise-world-contact-us-field"><input type="email"
					                                                    id="advertise-world-email-email"
					                                                    name="advertise_world_support[email]"
					                                                    value="<?php echo $email; ?>"/></td>
				</tr>
				<tr>
					<td class="advertise-world-contact-us-label"><label>your name</label></td>
					<td class="advertise-world-contact-us-field"><input type="text"
					                                                    id="advertise-world-email-name"
					                                                    name="advertise_world_support[name]"
					                                                    value="<?php echo $name; ?>"/></td>
				</tr>
				<tr>
					<td class="advertise-world-contact-us-label"><label>your message</label></td>
					<td class="advertise-world-contact-us-field"><textarea
							id="advertise-world-email-body"
							name="advertise_world_support[message]"><?php echo $message; ?></textarea></td>
				</tr>
			</table>
			<input type="submit" class="button button-primary" value="<?php esc_attr_e( 'Send' ); ?>">
		</form>

	</div>
	</div><?php
}

/**
 * Validates changes to advertise-world-wp-options-account.
 *
 * Makes sure that the email enterd is a valid Advertise World account.
 *
 * @since 1.0.0
 *
 * @param array $input Form data from dashboard containing string account email
 *
 * @return array Account information
 */
function advertise_world_wp_options_account_validate( $input ) {

	$host = "www.advertiseworld.com";
	$protocol = "https";

	/*
	$alternate_hostname = @fopen( "/var/www/kritter_adsserver.conf", 'r' );
	if ($alternate_hostname) {
		@fclose($alternate_hostname);
		$host = "test.advertiseworld.com";
		$protocol = "http";
	}*/

	$api_url = $protocol."://".$host."/portal/api/v1/publisher/adspaces/list?email=" . $input['account'];

	require_once ( dirname(__FILE__) . "/../advertise-world-wp.php" );
	$result = Advertise_World_Get_URL( $api_url );
	if ($result == FALSE) {
        add_settings_error(
            'network-error',
            'settings_updated',
            'Error connecting to Advertise World Server. Operation Timed Out.',
            'error' );
    }

	$result = json_decode( $result, true );

	if ( ! $result['accountExist'] ) {
		add_settings_error(
			'account-invalid',
			'settings_updated',
			'The Email you have enterd is not linked to a valid Advertise World account!',
			'error' );
	}

	return $input;
}


/**
 * Fires when admin scripts have been loaded.
 *
 * Adds custom css to admin section.
 *
 * @since 1.0.0
 */
function advertise_world_wp_options_dashboard_enqueue_style() {
	?><style><?php
	readfile ( dirname( __FILE__ ) . "/css/advertise-world-dashboard.css" );
	?></style><?php
}

add_action( 'admin_enqueue_scripts', 'advertise_world_wp_options_dashboard_enqueue_style' );